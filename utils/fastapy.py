...
def write_handle(handle, record):
    ...
    data = str(record.seq)  # Catches sequence being None
    assert "\n" not in data
    assert "\r" not in data
    for i in range(0, len(data), 60):
        handle.write(data[i : i + 60] + "\n")
...
def write(filename, records):
    ...
    Args:
        filename: a name or pathlib.Path of a file where u want to write FASTA file
    Returns:
        void.
